...
public class TopDeathsStatistic extends Statistic<UUID, Integer> {
     ...
     */
    public TopDeathsStatistic(MineHutCorePlugin plugin, Set<UUID> uuidSet) {
        super("mhc-top-deaths");
        this.plugin = plugin;
    ...
    protected Integer get(UUID key) {
        return plugin.getUserCache().getCache().get(key).join().getDeaths();
    }
